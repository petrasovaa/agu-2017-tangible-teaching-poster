...
\definebackgroundstyle{Rays}{
    \draw[line width=0pt, top color=backgroundcolor!50, bottom
    color=backgroundcolor!70!black] (bottomleft) rectangle (topright);
    %
    \begin{scope}
        \foreach \a in {5,10,...,200}{%
            \draw[backgroundcolor, line width=0.15cm](bottomleft) --
            ($(bottomleft)!1!(bottomleft)+(\a:200)$);%
        }
        \foreach \i in {1,2,...,1000}{%
            \begin{scope}[shift={($(rand*100,rand*300)$)}]
                \draw[backgroundcolor!30!, line width=0.1cm] (0,0) circle (4);
            \end{scope}
        }
    \end{scope}
}
